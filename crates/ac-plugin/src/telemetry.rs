...
fn sample(client: &Client) -> Value {
    ...
    json!({
        ...
        "coin": client.purse(),
        // What the town-run rule last found short and how long ago: [age s, [[name, have, keep, urgent]]].
        "needs": client.supplies_seen(Instant::now()).map(|(age, lines)| json!([age.round(), lines])),
