...
/// The lines outside every `#[cfg(test)]` item, each with its 1-based number, so a file reads the
/// same whether its tests sit in it or in a `tests.rs` beside it. An item is found at whatever
/// indent it sits at, not only at column 0: test-only helpers live inside `impl` blocks, and
/// counting those as production makes the naming gate read a test name it promised to skip.
/// rustfmt lines a braced item's closing brace up with its attribute, which ends it; an unbraced
/// one ends at its `;`.
pub fn production_lines(text: &str) -> Vec<(usize, &str)> {
    ...
    while at < lines.len() {
        ...
        // The item is closed at the attribute's own indent, wherever it sits.
        let deep = indent(lines[at]);
        let shut = format!("{}}}", " ".repeat(deep));
        at += 1;
        while lines
            .get(at)
        ...
        if lines.get(at).is_some_and(|l| l.trim_end().ends_with('{')) {
            at += 1;
            while at < lines.len() && lines[at].trim_end() != shut {
                at += 1;
            }
        } else {
